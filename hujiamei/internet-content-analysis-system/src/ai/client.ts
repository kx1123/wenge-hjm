...
export class AIAnalyzer {
  ...
  private async callAI(prompt: string): Promise<string> {
    ...
    try {
      ...
      if (error.response) {
        ...
        throw new Error(`AI API调用失败: ${error.response.data?.message || error.response.statusText}`)
      }
      throw error
    }
  }
  ...
  async analyzeSentiment(options: AnalyzeOptions): Promise<SentimentAnalysisResult> {
    ...
    try {
      ...
      try {
        ...
        console.warn('JSON解析失败，尝试文本解析:', parseError, '原始响应:', cleanedResponse)
        // 如果不是JSON，尝试文本解析
        const sentiment = cleanedResponse.toLowerCase()
        let score = 50
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          return parsed.filter((k) => typeof k === 'string' && k.length > 0).slice(0, 5)
        }
      } catch {
        // 如果不是JSON，尝试提取引号内的内容
  ...
  async chat(messages: ChatMessage[]): Promise<string> {
    ...
    try {
      ...
      // 构建通义千问格式的消息
      const qwenMessages = [systemMessage, ...messages].map((m) => ({
